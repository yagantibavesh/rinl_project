...
export default function NewRequest() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = { ...form };
      if (!["purchase","budget"].includes(form.type)) delete payload.amount;
    ...
    return (
      <div className="max-w-lg mx-auto animate-fade-in">
        <div className="card p-10 text-center">
          <div className="w-16 h-16 rounded-full bg-green-500/15 border-2 border-green-500/50
          ...
          <p className="font-mono text-xs text-molten-400 bg-molten-500/10 border border-molten-500/20
                        inline-block px-3 py-1 rounded-full mt-2">{success.requestId}</p>
          <div className="mt-6 flex gap-3 justify-center">
            <button onClick={() => navigate("/my-requests")} className="btn-primary">
              View My Requests
            </button>
            <button onClick={() => { setSuccess(null); setForm({ type:"", title:"", description:"", priority:"medium", department: user?.department||"", amount:"" }); }}
              className="btn-ghost">
              New Request
            </button>
          </div>
              ...
              <button
                ...
                className={`flex items-center gap-2 p-3 rounded-lg border transition-all text-left
                  ${form.type === value
                    ...
                    : "border-forge-700 bg-forge-800 text-slate-400 hover:border-forge-600 hover:text-slate-300"
                  }`}
              >
            ...
            {PRIORITIES.map(({ value, label, desc, color }) => (
              <button
                ...
                className={`p-3 rounded-lg border transition-all text-left
                  ${form.priority === value ? color + " border-2" : "border-forge-700 bg-forge-800 text-slate-500 hover:border-forge-600"}`}
              >
